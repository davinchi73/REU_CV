...
# person class
class Person:
    def __init__(self, person_id, frame_detected):
        self.person_id = person_id
        self.temperatures = deque(maxlen=5)
        self.last_seen = frame_detected
...
# detect if person is close
def is_close(new_id, people, threshold=50):
    (x2, y2, w2, h2) = new_id
    for index, person in enumerate(people):
        (x1, y1, w1, h1) = person.person_id
...
while True:
    ...
    if frame_counter % 90 == 0:
        ...
        for (x, y, w, h) in faces:
            ...
            # Check if the detected face is close to any existing person
            person_index = is_close(person_id, people)
            if person_index is not None:
                person = people[person_index]
                person.person_id = person_id  # Update the person's location
                person.last_seen = frame_counter  # Update the last seen frame
            else:
                person = Person(person_id, frame_counter)
            ...
            for (nx, ny, nw, nh) in noses:
                ...
                nose_temperature = pixel_to_temperature(nose_temperature_pixel_value, min_temp=16, max_temp=35)
                ...
                # add the temp to the current person
                person.temperatures.append(nose_temperature)
    ...
    # Remove people who haven't been detected for 180 frames
    people = [person for person in people if frame_counter - person.last_seen <= 180]

    # Print the people list every 180 frames
    if frame_counter % 180 == 0:
        print("People list:", [(person.person_id, list(person.temperatures)) for person in people])
